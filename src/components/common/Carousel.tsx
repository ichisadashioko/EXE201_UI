...
import type { PetPicture } from "../../types/pets";

const Carousel = ({ images, id }: { images: PetPicture[]; id?: number }) => {
    ...
    <div className="flex flex-col items-center bg-pink-50 p-6 rounded-xl">
      ...
      >
        {images.map((img, idx) => (
          <div
            key={idx}
            className={`flex-shrink-0 w-full max-w-[300px] relative inline-block snap-center rounded-xl overflow-hidden shadow-lg ${
              idx === current ? "h-90" : "h-80"
            }`}
          >
            <img
              src={img.url}
        ...
        {images.map((_, idx) => (
          <div
            ...
            className={` h-2 rounded-full transition-all ${
              idx === current ? "bg-pink-500 w-8" : "bg-pink-200 w-2"
            }`}
          />
        ))}
      </div>

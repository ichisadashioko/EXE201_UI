...
export default function UsersMatchListView({
  me,
  matches,
  ...
  if (!matches || matches.length === 0) {
    return (
      <div style={{ textAlign: "center", padding: "40px", color: "#666" }}>
        ...
        <p>
          When you and another user like each other's pets, you'll see them
          here.
        </p>
      </div>
    );
  ...
  return (
    <div
      style={{ fontFamily: "sans-serif", maxWidth: "700px", margin: "auto" }}
    >
      <h1 style={{ textAlign: "center" }}>Your Matches</h1>
      {matches.map((match) => {
        // Determine who the other user is
        const otherUser =
          match.user_a.id === me.id ? match.user_b : match.user_a;

        // Determine which list of pets belongs to the current user and which to the other user
        const pet_that_i_liked_list =
          match.user_a.id === me.id
            ? match.user_a_liked_pets
            : match.user_b_liked_pets;
        const my_pets_that_the_other_liked_list =
          match.user_a.id === me.id
            ? match.user_b_liked_pets
            : match.user_a_liked_pets;
        ...
        return (
          <div
            key={`${match.user_a.id}-${match.user_b.id}`}
            style={{
          ...
          >
            ...
            <p style={{ color: "#666", fontSize: "0.9em", marginTop: "-10px" }}>
              Matched on:{" "}
              {new Date(match.creation_time * 1000).toLocaleDateString()}
            </p>
            ...
            >
              <div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap" }}>
                  {my_pets_that_the_other_liked_list.map((pet) => (
                    <PetDisplay key={pet.id} pet={pet} />
                  ))}
                </div>
              </div>
              <div>
